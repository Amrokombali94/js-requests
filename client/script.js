...
sayHelloButton.addEventListener('mouseover', changeButtonBack)
...
const repeatMyParam = (repeat) => {
    //YOUR CODE HERE

    axios.get(`http://localhost:3000/repeat/Jojo`)
    .then(res =>{
...
// CODE HERE
const queryF = () =>{
    axios.get('http://localhost:3000/query-test/?fruit=Banana pickles tometo')
        .then(res =>{
            console.log(res.data)
        })
}

document.getElementById('query-button').addEventListener('click', queryF)
...
*/
//let pele = doucment.createElement('p')
// pele.textContent = res.data[i]
//pele.appendChild('body)
...
const createFood = (e) =>{
    e.preventDefault()
    let foodInput = document.querySelector('input[type="text"]')
    const body =   {
    ...
    axios.post('http://localhost:3000/food', body)
        .then(res =>{
            for(let i =0; i < res.data.length; i++){
                ...
                pele2.textContent = res.data[i]
    
                body1.appendChild(pele2)
...
document.querySelector('input[type="submit"]').addEventListener('click', createFood)